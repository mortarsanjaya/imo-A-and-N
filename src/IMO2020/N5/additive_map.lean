...
{ coe := λ f, f.to_fun,
  coe_injective' := λ f g h, by cases f; cases g; simp only []; exact h }
...
begin
  ...
  rw [mul_zero, hb],
  rw [f.map_mul ha0 hb0, g.map_mul ha0 hb0, ha, hb]
end
...
⟨λ f g, ⟨(λ n, f n + g n),
  ...
  λ x y hx hy, by rw [f.map_mul hx hy, g.map_mul hx hy, add_add_add_comm]⟩⟩
...
{ add_comm := λ f g, by ext; rw [add_apply, add_apply, add_comm],
  add_assoc := λ f g h, by ext; iterate 4 { rw add_apply }; rw add_assoc,
...
@[simp] theorem map_pow (f : additive_map M) : ∀ x n : ℕ, f (x ^ n) = n • f x
| _ 0 := by rw [pow_zero, zero_nsmul, map_one]
| 0 (n+1) := by rw [zero_pow n.succ_pos, map_zero, nsmul_zero]
| (x+1) (n+1) := by rw [pow_succ', succ_nsmul', ← map_pow,
  f.map_mul (pow_ne_zero _ x.succ_ne_zero) x.succ_ne_zero]
...
theorem map_factorization (f : additive_map M) :
  ∀ n : ℕ, f n = n.factorization.sum (λ p k, k • f p) :=
begin
  refine nat.rec_on_mul _ _ (λ p hp, _) (λ a b ha hb, _),
  rw [nat.factorization_zero, finsupp.sum_zero_index, map_zero],
  rw [nat.factorization_one, finsupp.sum_zero_index, map_one],
  ...
  rcases eq_or_ne b 0 with rfl | hb0,
  rw [mul_zero, hb],
  rw [f.map_mul ha0 hb0, nat.factorization_mul ha0 hb0, finsupp.sum_add_index, ← ha, ← hb],
  rintros c -; exact zero_nsmul (f c),
  rintros c - d1 d2; rw add_nsmul
end

theorem zero_iff_primes {f : additive_map M} : f = 0 ↔ ∀ p : ℕ, p.prime → f p = 0 :=
  by rw ext_iff_primes; simp_rw additive_map.zero_apply
...
⟨λ a f, ⟨λ x, a • f x,
  ...
  λ x y hx hy, by rw [f.map_mul hx hy, smul_add]⟩⟩
...
{ one_smul := λ f, by ext; rw [smul_def, one_smul],
  mul_smul := λ x y f, by ext; iterate 3 { rw smul_def }; rw mul_smul }

instance : distrib_mul_action α (additive_map M) :=
{ smul_add := λ x f g, by ext; rw [smul_def, add_apply, add_apply, smul_add, smul_def, smul_def],
...
⟨λ f, ⟨(λ n, -(f n)),
  ...
  λ x y hx hy, by rw [f.map_mul hx hy, neg_add]⟩⟩

@[simp] theorem neg_apply {f : additive_map G} {n : ℕ} : (-f) n = -(f n) := rfl

instance : has_sub (additive_map G) :=
⟨λ f g, ⟨(λ n, f n - g n),
  by rw [map_zero, map_zero, sub_zero],
  by rw [map_one, map_one, sub_zero],
  λ x y hx hy, by rw [f.map_mul hx hy, g.map_mul hx hy, add_sub_add_comm]⟩⟩
