...
* `pcop_part_hom M p : strong_map M p →+ pcop_domain M p`, the map sending `f` to the
    `p`-coprime part `χ` of `f`, defined by `χ(n) = f(n)` for each `1 ≤ n < p`.
* `pmix_inv_hom M p`, the representation of `f` in `M × pcop_domain M p`.
...
* `pmix_equiv M p` is the isomorphism `(M × pcop_domain M p) ≃+ strong_map M p`
...
private theorem pstrong_mod_p (f : strong_map M p) {k : ℕ} (hkp : ¬(p : ℕ) ∣ k) : f k = f (k % p) :=
  pstrong_mod_p_induct f (f.strong k) hkp (nat.lt_pow_self (fact.out p.prime).one_lt k)
...
private theorem pstrong_val (f : strong_map M p) {x : ℕ} (h : x ≠ 0) :
  ...
  let hp := fact.out p.prime in by rw [zmod.pcop_part, dif_neg h, zmod.coe_unit_of_coprime,
    zmod.val_nat_cast, ← pstrong_mod_p f (nat.not_dvd_p_pcop hp h), ← map_pow_smul f hp.ne_zero,
    ← map_mul_add f (pow_ne_zero _ hp.ne_zero) (nat.pcop_ne_zero p h),
    ← nat.factorization_def x hp, nat.eq_mul_ppow_pcop hp x]
...
lemma ppow_part_hom_of_pcop (χ : pcop_domain M p) : ppow_part_hom M p (pcop_hom M p χ) = 0 :=
...
begin
  ...
  { rw [pcop_part_fn, units.coe_neg_one, this, f.strong 1 _ 1 _ one_ne_zero, map_one_zero],
...
def pcop_part (f : strong_map M p) : pcop_domain M p :=
...
lemma pcop_part_zero : pcop_part M p 0 = 0 :=
  by ext; rw [pcop_part_apply_eq_fn, zero_apply, pcop_domain.zero_apply]

lemma pcop_part_add (f g : strong_map M p) :
  pcop_part M p (f + g) = pcop_part M p f + pcop_part M p g :=
  by ext; simp only [pcop_part_apply_eq_fn, pcop_domain.add_apply, add_apply]

lemma pcop_part_eq_iff (f : strong_map M p) (χ : pcop_domain M p) :
  pcop_part M p f = χ ↔ ∀ n : ℕ, n ≠ 0 → n < p → f n = χ (zmod.pcop_part p n) :=
begin
  have hp := fact.out p.prime,
  split,
  { rintros rfl n h h0,
    rw [pcop_part_apply_eq_fn, zmod.pcop_val p h, nat.eq_pcop_of_coprime hp h, nat.mod_eq_of_lt h0],
  ...
  { intros h; ext,
    ...
    rw [zmod.pcop_val _ h0, nat.eq_pcop_of_coprime hp h0, nat.mod_eq_of_lt h1],
...
lemma pcop_part_eq_zero_iff (f : strong_map M p) :
  ...
  by rw pcop_part_eq_iff; simp only [pcop_domain.zero_apply]

/-- The `p`-coprime part of a strong map -/
def pcop_part_hom : strong_map M p →+ pcop_domain M p :=
...
lemma pcop_part_hom_eq_iff (f : strong_map M p) (χ : pcop_domain M p) :
...
lemma pcop_part_hom_of_pcop (χ : pcop_domain M p) : pcop_part_hom M p (pcop_hom M p χ) = χ :=
begin
...
def pmix_inv_hom : strong_map M p →+ M × pcop_domain M p :=
...
begin
  ...
  rw [pmix_inv_hom_pair, pmix_hom_apply_ne_zero M p _ _ h,
      ppow_part_hom_apply, pcop_part_hom_apply_eq_fn, ← pstrong_val f h]
end
...
/-- The equivalence between `M × pcop_domain M p` and `strong_map M p` -/
def pmix_equiv : M × pcop_domain M p ≃+ strong_map M p :=
