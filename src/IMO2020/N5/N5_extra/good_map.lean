...
def good (n : ℕ) (f : ℕ → α) := ∀ a b : ℕ, a ≠ 0 → b ≠ 0 → a + b = n → f a = f b
...
lemma good_factor {n : ℕ} (h : n ≠ 0) (h0 : good n f) {d : ℕ} (h1 : d ∣ n) : good d f :=
begin
...
begin
  ...
  { rw [← not_lt, ← nat.succ_le_iff]; intros h2,
    exact h1 (good_factor f (pow_ne_zero k hp.ne_zero) h0 (pow_dvd_pow p h2)) },
