...
lemma fn_lem1_2 : f 1 = 1 :=
  by rw [← zero_add (1 : F), feq2, fn_lem1_1 feq1 feq2]

lemma fn_lem1_3 (x : F) : f x = 0 ↔ x = 0 :=
begin
  rw iff.comm; split,
  rintros rfl; exact fn_lem1_1 feq1 feq2,
  intros h; by_contra h0,
  have h1 := feq1 x x⁻¹,
  rw [h, zero_mul, fn_lem1_1 feq1 feq2, zero_add, mul_inv_cancel h0,
...
lemma fn_lem2_2 (x : F) : f x = 1 ↔ x = 1 :=
  by rw [← sub_eq_zero, sub_eq_add, ← feq2, fn_lem1_3 feq1 feq2, ← sub_eq_add, sub_eq_zero]

lemma fn_lem2_3 (x : F) : f (x⁻¹) = (f x)⁻¹ :=
begin
  by_cases h : x = 0,
...
lemma fn_lem2_4 (x y : F) : f (f x * f y) + f (x + y) + 1 = f ((x + 1) * (y + 1)) :=
  by rw [feq1, ← feq2, ← mul_add_one, add_assoc, ← add_one_mul]

lemma fn_lem2_5 (x y : F) : f ((f x + 1) * (f y + 1)) + 1 = f (x * y) + f (x + y) :=
begin
...
lemma fn_lem2_6 {a b : F} (h : f a = f b) (x : F) : f (a * x) + f (a + x) = f(b * x) + f (b + x) :=
  by rw [← fn_lem2_5 feq1 feq2, h, fn_lem2_5 feq1 feq2]

lemma fn_lem2_7 {a b c d : F} (h : f a = f b) (h0 : f c = f d) :
  f (a * c) + f (a + c) = f(b * d) + f (b + d) :=
  by rw [fn_lem2_6 feq1 feq2 h, add_comm b, mul_comm,
