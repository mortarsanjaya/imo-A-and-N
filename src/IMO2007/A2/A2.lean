...
begin
  ...
  ---- `f(N) ≤ N + 1` for all `N : ℕ`.
  { clear h; rcases h0 with ⟨f, h, rfl⟩,
    generalize hK : N + 1 = K,
    rw [← not_lt, ← nat.add_one_le_iff]; intros h0,

    -- We prove that `f(mK) ≥ mf(K)` for all `m : ℕ` here.
  ...
  ---- Construction of good `f` with `f(N) = k`, `k ≤ N`.
  -- We have already substituted `N = k + c` here, and we do not need `N > 0`.
  { refine ⟨λ n, n - c, λ m n, _, nat.add_sub_cancel k c⟩,
    clear h k; dsimp only []; cases le_total n c with h h,
  ...
  ---- Construction of good `f` with `f(N) = N + 1`, `N > 0`.
  { refine ⟨λ n, n + ite (N + 1 ∣ n + 1) 1 0, λ m n, _, by rw if_pos (dvd_refl _)⟩,
    rw [← add_lt_add_iff_right 1, zero_add] at h,
    generalize_hyp : N + 1 = M at h ⊢,
    dsimp only []; clear N,
