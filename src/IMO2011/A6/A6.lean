...
variables {F : Type*} [linear_ordered_comm_ring F]
...
private lemma lem1 (x : F) : x * f x ≤ 0 :=
begin
  revert x; suffices : ∀ t x : F, f t ≤ (t - x) * f x + f (f x),
...
/-- Final solution -/
theorem final_solution (x : F) (h : x ≤ 0) : f x = 0 :=
begin
  have h0 : ∀ y : F, y < 0 → f y = 0 :=
