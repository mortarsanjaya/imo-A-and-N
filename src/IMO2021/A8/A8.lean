...
begin
  ...
  ring_nf at h,
  have h0 := h (-(x / c ^ 3)),
  rwa [neg_sq, h, neg_mul, div_mul_cancel x (pow_ne_zero 3 c_ne_0), eq_comm] at h0
end
...
begin
  ...
  replace h := (lem2_1 feq t c h (-t ^ 2)).symm,
...
begin
  intros h,
  rw [bit0, ← eq_neg_iff_add_eq_zero, ← f1_eq_1, ← thm3_1 feq f_inj f0_eq_0] at h,
  replace h := f_inj h,
