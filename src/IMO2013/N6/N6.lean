...
begin
  ...
  { have h1 : 0 < 1 - 2 * f (1 / 2) := by rw sub_pos;
    ...
    rw [← sub_eq_add_neg, eq_div_iff, div_sub', coe_coe, ← int.cast_coe_nat, ← coe_coe b,
        h1, int.cast_sub, int.cast_mul, int.cast_one, int.cast_two, div_mul_comm, mul_one],
    exact two_ne_zero,
  ...
  { have h1 : 0 < 2 * f (1 / 2) - 1 := begin
    ...
    rw [eq_div_iff h2, div_add', coe_coe, ← int.cast_coe_nat, ← coe_coe b, h1, int.cast_sub,
        int.cast_sub, sub_mul, add_sub_left_comm, int.cast_one, mul_two (1 : ℚ), int.cast_mul,
        sub_add_cancel', ← sub_eq_add_neg, int.cast_two, mul_comm (2 : ℚ), div_mul_comm, mul_one],
    exact two_ne_zero,
