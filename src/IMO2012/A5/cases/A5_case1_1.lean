import IMO2012.A5.cases.A5_case1_lemmas
...
variables [is_domain S] {f : R → S} (h : good f)
  (h0 : f 0 = -1) (h1 : f (-1) ≠ 0) (h2 : f (-1) = -2)
include h h0 h1 h2

private lemma case1_1_lem1 (x : R) : f (x - 1) - f (x + 1) = -2 :=
  (eq_or_ne (f x) 0).elim
  (λ h3, let h4 := case1_map_eq_zero_imp h h0 h1 h3 in by rw [h4.1, h4.2, ← neg_add', bit0])
  (λ h3, (case1_map_ne_zero_imp h h0 h1 h3).trans h2)

private lemma case1_1_lem2 (x : R) : f (-x) = -(f x + 2) :=
  by rw [case1_map_neg h h0 h1, neg_add_rev, ← case1_1_lem1 h h0 h1 h2 (x + 1),
    add_sub_cancel, ← sub_eq_add_neg, sub_sub_cancel_left, bit0, add_assoc]

private lemma case1_1_lem3 (x : R) : f (x + 1) = f x + 1 :=
  by have h3 := map_neg_sub_map2 h x; rwa [case1_1_lem2 h h0 h1 h2, ← neg_add',
    add_right_comm, ← mul_two, ← add_one_mul, ← mul_neg, ← h2, mul_left_inj' h1, eq_comm] at h3

private lemma case1_1_lem4 (x y : R) : f (x + y) = f x + f y + 1 :=
  let h4 := case1_1_lem2 h h0 h1 h2 in
  by have h3 := case1_map_add_main_equality h x y;
  rwa [h2, neg_mul_neg, h4, h4, neg_mul_neg, add_mul, mul_add, mul_add,
